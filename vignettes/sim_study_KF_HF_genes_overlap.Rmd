...
for(i in 1:length(overlap.prop.vec)){
  dat0 <- data.list[[i]]
  ### iasva 
  hfactors_iasva_list[[i]] <- iasva(t(dat0), kfactor, num.sv=1, permute = F)$sv
  ## Estimate batch with pca
  ldat0 = log(dat0 + 1)
...
iasva.avg.cor.mat <- matrix(rep(0,16),nrow=4)
pca.avg.cor.mat <- matrix(rep(0,16),nrow=4)
usva.avg.cor.mat <- matrix(rep(0,16),nrow=4)
ssva.avg.cor.mat <- matrix(rep(0,16),nrow=4)
for(i in 1:num.sims){
