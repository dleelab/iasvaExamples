...
#raw.counts <- counts # this is for cell cycell prediction using SCRAN
...
It is well known that the number of detected genes in each cell explains a very large portion of variability in scRNA-Seq data ([Hicks et. al. 2015 BioRxiv](http://biorxiv.org/content/early/2015/08/25/025528), [McDavid et. al. 2016 Nature Biotechnology](http://www.nature.com/nbt/journal/v34/n6/full/nbt.3498.html)). Frequently, the first principal component of log-transformed scRNA-Seq read counts is highly correlated with the number of detected genes (e.g., r > 0.9). Here, we calculate the number of detected genes for glioblastoma cells, which will be used as an known factor in the IA-SVA analyses. 
...
set.seed(3445)
#mod <- model.matrix(~Num_Detected_Genes+Subtype+Percent_Mito)
#mod <- model.matrix(~Num_Detected_Genes+Subtype)
...
pairs(iasva.sv[,1:5], main="IA-SVA", pch=21, col=color.vec[Cluster], bg=color.vec[Cluster], oma=c(4,4,6,14))
...
#legend("bottomright", levels(Cluster), fill=color.vec, bty="n")
...
As shown in the above figure, SV1 clearly separates glioblastoma cells into two groups: 13 cells (marked in red) and the rest of the cells (marked in blue). Note that SV1 is moderately correlated with the geometric library size (|r|=0.44).


## Find marker genes for the detected heterogeneity (SV1).
Here, using the find_markers() function we find marker genes (n=119 genes) that are significantly associated with SV1 (multiple testing adjusted p-value < 0.05, default significance cutoff, and R-squared value > 0.3).  
```{r find_markers_SV1, echo=TRUE, fig.width=6, fig.height=16}
# try different R2 thresholds
pdf(paste0(path.dir,"Clustering_analyses_figure3_sv1.pdf"))
r2.results <- study_R2(t(counts), iasva.sv,selected.svs=1, no.clusters=2)
dev.off()

marker.counts.SV1 <- find_markers(t(counts), as.matrix(iasva.sv[,c(1)]), rsq.cutoff = 0.4)
marker.counts.SV1.long <- find_markers(t(counts), as.matrix(iasva.sv[,c(1)]), rsq.cutoff = 0.3)
nrow(marker.counts.SV1) #62 74
nrow(marker.counts.SV1.long) #119 74
...
pheatmap(log(marker.counts.SV1+1), show_colnames =FALSE, clustering_method = "ward.D2",cutree_cols = 2,annotation_col = anno.col2,
         annotation_colors = anno.colors)

pheatmap(log(marker.counts.SV1.long+1), show_colnames =FALSE, clustering_method = "ward.D2",cutree_cols = 2,annotation_col = anno.col2,
         annotation_colors = anno.colors)

gene.list <- rownames(marker.counts.SV1)
write.table(gene.list, file = paste0(path.dir,"CC_genes.short.txt"), col.names =F, row.names = F, quote = F)

gene.list <- rownames(marker.counts.SV1.long)
write.table(gene.list, file = paste0(path.dir,"CC_genes.long.txt"), col.names =F, row.names = F, quote = F)
...
plot(tsne.res$Y, main="tSNE", xlab="Dim1", ylab="Dim2", pch=21, col=color.vec[Cluster], bg=color.vec[Cluster], oma=c(4,4,6,12))
...
set.seed(3452)
tsne.res <- Rtsne(unique(t(log(marker.counts.SV1.long+1))), dims = 2, perplexity = 20)

plot(tsne.res$Y, main="IA-SVA + tSNE", xlab="tSNE Dim1", ylab="tSNE Dim2", pch=21, col=color.vec[Cluster], bg=color.vec[Cluster], oma=c(4,4,6,12))
...
pairs(pca.res[,1:5], main="PCA", pch=21, col=color.vec[Cluster], bg=color.vec[Cluster],
      oma=c(4,4,6,14))
legend("right", levels(Cluster), fill=color.vec, bty="n")

plot(pca.res[,1:2], main="PCA", pch=21, xlab="PC1", ylab="PC2", col=color.vec[Cluster], bg=color.vec[Cluster])
...
#mod1 <- model.matrix(~Num_Detected_Genes+Subtype+Percent_Mito)
#mod1 <- model.matrix(~Num_Detected_Genes+Subtype)
mod1 <- model.matrix(~Geo_Lib)
mod0 <- cbind(mod1[,1])

sva.res = svaseq(counts,mod1,mod0, n.sv=5)$sv

pairs(sva.res[,1:5], main="SVA", pch=21, col=color.vec[Cluster], bg=color.vec[Cluster], oma=c(4,4,6,14)) #4,4,6,12
...
pdf(file=paste0(path.dir,"Patel_Glioblastoma_MGH30_CellCycle_Figure3ABCD.pdf"), width=5, height=8)
layout(matrix(c(1,2,3,4,5,5), nrow=3, ncol=2, byrow=TRUE))
plot(iasva.sv[,1:2], main="IA-SVA", pch=21, xlab="SV1", ylab="SV2", col=color.vec[Cluster], bg=color.vec[Cluster], oma=c(4,4,6,12))
legend("topright", levels(Cluster), border="white", fill=color.vec, bty="n")
plot(pca.res[,1:2], main="PCA", pch=21, xlab="PC1", ylab="PC2", col=color.vec[Cluster], bg=color.vec[Cluster])
...
pheatmap(log(marker.counts.SV1.long+1), show_colnames =FALSE, clustering_method = "ward.D2",cutree_cols = 2,annotation_col = anno.col2,
         annotation_colors = anno.colors,
         filename=paste0(path.dir,"Patel_Glioblastoma_MGH30_iasva_SV1_genes_rsqcutoff0.3_pheatmap_iasvaV0.95_Figure3F.pdf"), width=6, height=16)

```


```{r gen_SV1_gene_list}
write.table(as.data.frame(rownames(marker.counts.SV1)), file=paste0(path.dir,"Patel_Glioblastoma_MGH30_Cellcycle_SV1_Genes_rsqcutoff0.4.txt"), quote=F,
              row.names=F, col.names=F, sep=" ")

write.table(as.data.frame(rownames(marker.counts.SV1.long)), file=paste0(path.dir,"Patel_Glioblastoma_MGH30_Cellcycle_SV1_Genes_rsqcutoff0.3.txt"), quote=F,
              row.names=F, col.names=F, sep=" ")
```
