...
## Load the islet scRNA-Seq data (1600 cells, >26K genes)
...
## Filter out low-expressed genes dataset reduced to 19K genes and 1600 cells
...
## Run Mohan's Method (CellView) to cluster islet cells
...
plot(rtsne_out$Y[,c(1,2)], main="CellView", xlab="Dimension 1", ylab="Dimension 2",pch=21, col=color.vec[Cell_Type], bg=color.vec[Cell_Type], oma=c(4,4,6,12))
legend("topright", levels(Cell_Type), border="white", fill=color.vec, bty="n")
```
## Run Seurat to cluster islet cells
```{r run_Seurat, echo=TRUE, fig.width=7, fig.height=7}
set.seed(12344)
seurat.obj <- CreateSeuratObject(raw.data=counts, min.cells=3, min.genes=200, project="Seurat_Comp")
...
# Filter cells
#seurat.obj <- FilterCells(object = seurat.obj, subset.names = c("nGene", "percent.mito"), 
...
#PrintPCA(object = seurat.obj, pcs.print = 1:5, genes.print = 5, use.full = FALSE)
#VizPCA(object = seurat.obj, pcs.use = 1:2)
#PCAPlot(object = seurat.obj, dim.1 = 1, dim.2 = 2)
#seurat.obj <- ProjectPCA(object = seurat.obj, do.print = FALSE)
#PCHeatmap(object = seurat.obj, pc.use = 1, cells.use = 500, do.balanced = TRUE, label.columns = FALSE)
#PCHeatmap(object = seurat.obj, pc.use = 1:12, cells.use = 500, do.balanced = TRUE, 
...
for (i in seq(0.1,0.9,0.05)){
  marker.counts <- find_markers(t(counts), as.matrix(iasva.sv[,2]), rsq.cutoff = i)
...
Here, we find marker genes significantly associated with SV1 and SV4 (multiple testing adjusted p-value < 0.05, default significance cutoff) and having a high R-squared value (R-squared > 0.3) using the find_markers() function.
...
dev.off()
marker.counts <- iasva::find_markers(t(counts), as.matrix(iasva.sv[,c(1)]),  rsq.cutoff = 0.3)
nrow(marker.counts) #57
marker.counts.long <- iasva::find_markers(t(counts), as.matrix(iasva.sv[,c(1)]),  rsq.cutoff = 0.2)
nrow(marker.counts.long) #123
...
pheatmap(log(marker.counts+1), show_colnames =FALSE, show_rownames = TRUE, clustering_method = "ward.D2",cutree_cols = 4,annotation_col = anno.col, annotation_colors=anno.colors)
```

## Find marker genes for SV3.
Here, we find marker genes significantly associated with SV1 and SV4 (multiple testing adjusted p-value < 0.05, default significance cutoff) and having a high R-squared value (R-squared > 0.3) using the find_markers() function.
```{r find_markers_SV3, echo=TRUE, fig.width=7, fig.height=10}
marker.counts.SV3 <- iasva::find_markers(t(counts), as.matrix(iasva.sv[,c(3)]),  rsq.cutoff = 0.3)
nrow(marker.counts.SV3)
marker.counts.SV3.long <- iasva::find_markers(t(counts), as.matrix(iasva.sv[,c(3)]),  rsq.cutoff = 0.2)
...
plot(tsne.res.iasva$Y[,1:2], main="IA-SVA", xlab="tSNE Dim1", ylab="tSNE Dim2", pch=21, col=color.vec[Cell_Type], bg=color.vec[Cell_Type], oma=c(4,4,6,12))
legend("topright", levels(Cell_Type), border="white", fill=color.vec, bty="n")
```
...
pheatmap(log(marker.counts+1), show_colnames =FALSE, show_rownames = TRUE, clustering_method = "ward.D2",cutree_cols = 4,annotation_col = anno.col, annotation_colors=anno.colors, filename="FigureS11_Xin_Islets_AllCells_IASVA_Markers_pheatmap.pdf", width=10, height=10)
```
