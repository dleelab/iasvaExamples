...
hf.mat <- cbind(unknown, res$sv[,1], pr.out$x[,1], sva.res[,1])
colnames(hf.mat) <- c("HF", "IASVA","PCA","SVA")
corrplot.mixed(abs(cor(hf.mat)), tl.pos="lt")
...
hf.mat <- cbind(unknown, res$sv[,1], pr.out$x[,1], sva.res[,1])
colnames(hf.mat) <- c("HF", "IA-SVA","PCA","SVA")
corrplot.mixed(abs(cor(hf.mat)), tl.pos="lt")
```
...
```{r sim1_corrected_high_corr, fig.width=6, fig.height=6}
# Setting up the known and unknown factors. 
set.seed(10000) 
...
Here, we simulated known and hidden factors to affect only 100 (5%) and 20 (1%) genes respectively. We simulated the 20 genes affected by the hidden factor to be also affected by the known factor. Similarly, the two factors are highly correlated (r=0.7).  
```{r sim1_corrected_high_corr_extreme, fig.width=6, fig.height=6}
# Setting up the known and unknown factors. 
set.seed(10000) 
